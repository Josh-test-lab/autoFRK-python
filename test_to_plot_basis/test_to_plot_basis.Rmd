...
for (k in 1:K) {
  bk = 0.2 * (k - 1)
  result = rbind(result, basis_function(s, bk, rk))
}
result = result[-1, ]
```
...
```{r}
# plot
plot(x = s, y = result[1, ], xlim = c(0, 1), ylim = c(0, 1), col = 1, main = "basis function", xlab = "S", ylab = "f(s)")
for (k in 2:K) {
